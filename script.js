...
sortedByBirthYear.sort(compareYears);
...
totalYearsLived = inventors.reduce((totalSum, years) => {
    return totalSum + (years.passed - years.year)
}, 0);
...
travelMethodCounts = travelMethods.reduce((travelWays, waysOfTravel) => {
    // console.log(travelWays[waysOfTravel]);
    if (travelWays[waysOfTravel]) {
        travelWays[waysOfTravel] = travelWays[waysOfTravel] + 1;
    } else {
        travelWays[waysOfTravel] = 1;
    }
    return travelWays;
}, {});
